...
export class LojaExterna {
  ...
  static schema = z.object({
    setor: z.enum(["Laranja", "Azul", "Vermelho", "Verde", "Amarelo", "Branco"]),
    ...
    gridArea: z.array(z.object({
      y: z.number().int(),
      x: z.number().int(),
    })),
  });

  constructor({ setor, bloco, numLoja, gridArea }: z.infer<typeof LojaExterna.schema>) {
  ...
  getBounds() {
    ...
    const lojaLeftBottom: [number, number] = [Math.min(...yValues), Math.min(...xValues)];
    const lojaRightTop: [number, number] = [Math.max(...yValues) + 1, Math.max(...xValues) + 1];
